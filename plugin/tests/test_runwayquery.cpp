...
RunwayQuery::BudgetRequest budgetRequest(int completedDays = 10, int reportedDays = 7)
{
...
QVariantMap budgetMap(const RunwayQuery::BudgetRequest &request)
{
    ...
    return {
        ...
        { QStringLiteral("valueClass"), ForecastContract::valueClassKey(request.valueClass) },
...
{
    const ForecastContract::Result result = RunwayQuery::quotaRunway(quotaRequest(), Now);
    ...
    QCOMPARE(result.predictedAt->toSecsSinceEpoch(), Now.addSecs(20 * 60).toSecsSinceEpoch());
...
void RunwayQueryTest::quotaNoConsumptionIsSafe()
{
    ...
    const ForecastContract::Result result = RunwayQuery::quotaRunway(request, Now);
    QVERIFY(result.isValid());
    QCOMPARE(result.state, ForecastContract::State::Safe);
...
{
    {
        ...
        QCOMPARE(RunwayQuery::quotaRunway(request, Now).reasonKey, QStringLiteral("insufficient_samples"));
    }
    {
        RunwayQuery::QuotaRequest request = quotaRequest();
        ...
        QCOMPARE(RunwayQuery::quotaRunway(request, Now).reasonKey, QStringLiteral("insufficient_span"));
    }
    {
        RunwayQuery::QuotaRequest request = quotaRequest();
        QCOMPARE(RunwayQuery::quotaRunway(request, Now.addSecs(16 * 60)).reasonKey, QStringLiteral("stale_data"));
    }
    {
        RunwayQuery::QuotaRequest request = quotaRequest();
        request.samples[2].remaining = 90.0;
        QCOMPARE(RunwayQuery::quotaRunway(request, Now).reasonKey, QStringLiteral("non_monotonic"));
    }
    {
        RunwayQuery::QuotaRequest request = quotaRequest();
        request.samples[1].resetAt = request.samples[1].resetAt.addSecs(60);
        QCOMPARE(RunwayQuery::quotaRunway(request, Now).reasonKey, QStringLiteral("reset_detected"));
    }
    {
        RunwayQuery::QuotaRequest request = quotaRequest();
        request.samples.last().remaining.reset();
        QCOMPARE(RunwayQuery::quotaRunway(request, Now).reasonKey, QStringLiteral("missing_value"));
    }
    {
        RunwayQuery::QuotaRequest request = quotaRequest();
        request.samples.last().source = QStringLiteral("self_tracked");
        QCOMPARE(RunwayQuery::quotaRunway(request, Now).reasonKey, QStringLiteral("unsupported_source"));
    }
}
...
void RunwayQueryTest::quotaSparseCoverageAndTimeZones()
{
    ...
    for (qsizetype index = 0; index < request.samples.size(); ++index) {
        request.samples[index].observedAt
            = Now.addSecs(minutes.at(index) * 60).toTimeZone(QTimeZone("Europe/Stockholm"));
    }
    const ForecastContract::Result result
        = RunwayQuery::quotaRunway(request, Now.toTimeZone(QTimeZone("America/New_York")));
    QVERIFY(result.isValid());
    ...
    for (qsizetype index = 0; index < dst.samples.size(); ++index) {
        dst.samples[index].observedAt
            = dstNow.addSecs((-30 + index * 10) * 60).toTimeZone(QTimeZone("America/New_York"));
        dst.samples[index].resetAt = reset.toTimeZone(QTimeZone("Europe/Stockholm"));
    }
    const ForecastContract::Result dstResult = RunwayQuery::quotaRunway(dst, dstNow);
...
{
    {
        ...
        QCOMPARE(RunwayQuery::budgetPacing(request, Now).reasonKey, QStringLiteral("insufficient_samples"));
    }
    {
        RunwayQuery::BudgetRequest request = budgetRequest(10, 6);
        QCOMPARE(RunwayQuery::budgetPacing(request, Now).reasonKey, QStringLiteral("insufficient_coverage"));
    }
    {
        RunwayQuery::BudgetRequest request = budgetRequest();
        request.days.last().currency = QStringLiteral("EUR");
        QCOMPARE(RunwayQuery::budgetPacing(request, Now).reasonKey, QStringLiteral("mixed_currency"));
    }
    {
        RunwayQuery::BudgetRequest request = budgetRequest();
        request.budgetCurrency = QStringLiteral("EUR");
        QCOMPARE(RunwayQuery::budgetPacing(request, Now).reasonKey, QStringLiteral("currency_mismatch"));
    }
    {
        RunwayQuery::BudgetRequest request = budgetRequest();
        request.days.last().valueClass = ForecastContract::ValueClass::Estimated;
        QCOMPARE(RunwayQuery::budgetPacing(request, Now).reasonKey, QStringLiteral("mixed_value_class"));
    }
    {
        RunwayQuery::BudgetRequest request = budgetRequest();
        request.budget = 0.0;
        QCOMPARE(RunwayQuery::budgetPacing(request, Now).reasonKey, QStringLiteral("missing_budget"));
    }
}
...
void RunwayQueryTest::budgetActualEstimatedSeparation()
{
    ...
    const ForecastContract::Result actualResult = RunwayQuery::budgetPacing(actual, Now);
    const ForecastContract::Result estimatedResult = RunwayQuery::budgetPacing(estimated, Now);
    ...
    const ForecastContract::Result strongResult = RunwayQuery::budgetPacing(budgetRequest(10, 10), Now);
...
void RunwayQueryTest::databaseQueryAndConnectionCleanup()
{
    ...
    const QString connection
        = QStringLiteral("runway_fixture_%1").arg(QUuid::createUuid().toString(QUuid::WithoutBraces));
    {
        QSqlDatabase database = QSqlDatabase::addDatabase(QStringLiteral("QSQLITE"), connection);
        database.setDatabaseName(path);
        QVERIFY(database.open());
        QSqlQuery query(database);
        QVERIFY(query.exec(QStringLiteral("CREATE TABLE observations("
        ...
        query.prepare(QStringLiteral("INSERT INTO observations(provider,observed_at_utc,metric_kind,"
        ...
        for (int index = 0; index < 4; ++index) {
            ...
            const QString observed
                = Now.addSecs((-30 + index * 10) * 60).toString(QStringLiteral("yyyy-MM-dd HH:mm:ss"));
            const QString reset = Now.addSecs(2 * 60 * 60).toString(QStringLiteral("yyyy-MM-dd HH:mm:ss"));
    ...
    const QVariantList results = RunwayQuery::execute({ { QStringLiteral("databasePath"), path },
        { QStringLiteral("generatedAt"), Now }, { QStringLiteral("quotaSources"), QVariantList { descriptor } } });
    QCOMPARE(results.size(), 1);
    const QVariantMap result = results.first().toMap();
    QVERIFY2(result.value(QStringLiteral("state")).toString() == QLatin1String("critical"),
    ...
    model.refreshWithQuery({ { QStringLiteral("databasePath"), path }, { QStringLiteral("generatedAt"), Now },
...
void RunwayQueryTest::asynchronousSupersessionAndCleanup()
{
    ...
    model.refreshWithQuery(
        { { QStringLiteral("generatedAt"), Now }, { QStringLiteral("quotaSeries"), QVariantList { quotaMap(slow) } } });

    RunwayQuery::BudgetRequest current = budgetRequest();
    current.sourceId = QStringLiteral("latest-budget");
    model.refreshWithQuery({ { QStringLiteral("generatedAt"), Now },
    ...
    QCOMPARE(model.rowCount(), 1);
    QCOMPARE(model.forecasts().first().toMap().value(QStringLiteral("sourceId")).toString(),
        QStringLiteral("latest-budget"));
    QCOMPARE(model.data(model.index(0, 0), GuardrailModel::SourceIdRole).toString(), QStringLiteral("latest-budget"));
    QVERIFY(!model.data(model.index(0, 0), GuardrailModel::EvidenceGradeRole).toString().isEmpty());
...
void RunwayQueryTest::performanceAtOneHundredThousandObservations()
{
    ...
    QVERIFY(result.isValid());
    QVERIFY2(elapsed <= 750, qPrintable(QStringLiteral("100k runway query took %1 ms").arg(elapsed)));
