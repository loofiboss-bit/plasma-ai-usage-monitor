...
ForecastContract::Result availableForecast(ForecastContract::State state)
{
    ...
    result.periodEnd = QDateTime(QDate(2026, 7, 29), QTime(12, 0), QTimeZone::UTC);
    ...
    result.generatedAt = QDateTime(QDate(2026, 7, 29), QTime(9, 0), QTimeZone::UTC);
    return result;
}
} // namespace
...
void ForecastContractTest::availableStates_data()
{
    ...
    QTest::newRow("safe") << int(ForecastContract::State::Safe) << QStringLiteral("safe");
    QTest::newRow("warning") << int(ForecastContract::State::Warning) << QStringLiteral("warning");
    QTest::newRow("critical") << int(ForecastContract::State::Critical) << QStringLiteral("critical");
}
...
void ForecastContractTest::availableStates()
{
    QFETCH(int, state);
    QFETCH(QString, key);
    ForecastContract::Result result = availableForecast(ForecastContract::State(state));
...
void ForecastContractTest::unavailableReasons()
{
    ...
    ForecastContract::Result result = availableForecast(ForecastContract::State::Unavailable);
    ...
    QString diagnostic;
    QVERIFY2(result.isValid(&diagnostic), qPrintable(diagnostic));
    const auto roundTrip = ForecastContract::fromVariantMap(result.toVariantMap(), &diagnostic);
    QVERIFY2(roundTrip.has_value(), qPrintable(diagnostic));
...
{
    ForecastContract::Result result = availableForecast(ForecastContract::State::Safe);
...
{
    ForecastContract::Result result = availableForecast(ForecastContract::State::Warning);
...
void ForecastContractTest::invalidContractsFailClosed()
{
    ...
    QVariantMap map = availableForecast(ForecastContract::State::Safe).toVariantMap();
...
void ForecastContractTest::stableIdentityAndTransitions()
{
    ...
    QCOMPARE(ForecastContract::transitionFor(State::Safe, State::Warning), QStringLiteral("warning"));
    QCOMPARE(ForecastContract::transitionFor(State::Warning, State::Warning), QString());
    QCOMPARE(ForecastContract::transitionFor(State::Warning, State::Critical), QStringLiteral("critical"));
    QCOMPARE(ForecastContract::transitionFor(State::Critical, State::Safe), QStringLiteral("recovered"));
    QCOMPARE(ForecastContract::transitionFor(State::Critical, State::Unavailable), QStringLiteral("recovered"));
    QCOMPARE(ForecastContract::transitionFor(State::Safe, State::Safe), QString());
}
