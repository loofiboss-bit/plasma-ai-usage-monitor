...
Google Antigravity is opt-in and Linux-only. Keep the Antigravity 2.x desktop app running and signed in. The widget detects the plan and reads live per-model quota plus shared five-hour and weekly windows from the validated localhost daemon over pinned TLS. It does not read prompts, conversations, OAuth tokens, or logs, and the in-memory daemon CSRF value never enters diagnostics, history, or exports. Plans, model access, and limits are taken from the live response rather than a static model matrix. Gemini web-app usage is a separate product quota and is not included.
