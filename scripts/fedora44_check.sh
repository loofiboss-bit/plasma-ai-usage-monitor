...
if python3 scripts/check_qml_registered_types.py; then ok "QML registered types"; else fail "QML registered type check failed" "Align plugin/CMakeLists.txt with declarative QML registration macros."; fi
if python3 scripts/check_provider_catalog.py; then ok "Provider Catalog v7"; else fail "Provider Catalog v7 invalid" "Update package/contents/catalog/providers-v4.json."; fi
...
if command -v appstreamcli >/dev/null 2>&1 && appstreamcli validate --no-net com.github.loofi.aiusagemonitor.metainfo.xml; then ok "AppStream metadata"; else fail "AppStream validation failed" "Install appstream and fix com.github.loofi.aiusagemonitor.metainfo.xml."; fi
